...
int 
json_get_key_type(const struct json_object *obj){
	struct lh_entry *type_entry = json_get_keyword_entry(json_object_get_object(obj),"type");
	//assume its an object(id=4) by default
	int key_type = json_type_object, i,size = json_type_string - json_type_null +2; // json-c types + "number" type
...
void
json_split_str(int keyword_position, int *return_array, int str_pos){
